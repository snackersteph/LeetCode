/**
 * @param {number[]} nums1
 * @param {number[]} nums2
 * @return {number[]}
 */
var intersect = function(nums1, nums2) {
  sortedNums1 = nums1.sort()
  sortedNums2 = nums2.sort()

  console.log("NUMS1: ", sortedNums1)
  console.log("NUMS2: ", sortedNums2)

  var i=0;
  var j=0;
  var k=0;

  while (i < nums1.length && j < nums2.length) {
    if (nums1[i] < nums2[j]) {
      i++
      console.log("first i::: ", nums1[i], nums2[i], i)
    } 
    else if (nums1[i] > nums2[j]) {
      j++
      console.log("second j::: ", nums1[i], nums2[i], j)
      
    }
    else {
      nums1[k] = nums1[i]
      console.log("third nums::: ", nums1[i])

      k++
      i++
      j++
      console.log("third j::: ", j)
      console.log("third k::: ", k)
    }
  }

  console.log("final K::: ", k)
  console.log("nums1.slice(0, k)", nums1.slice(0, k))
  return nums1.slice(0, k)
};

var nums1 = [61,24,20,58,95,53,17,32,45,85,70,20,83,62,35,89,5,95,12,86,58,77,30,64,46,13,5,92,67,40,20,38,31,18,89,85,7,30,67,34,62,35,47,98,3,41,53,26,66,40,54,44,57,46,70,60,4,63,82,42,65,59,17,98,29,72,1,96,82,66,98,6,92,31,43,81,88,60,10,55,66,82,0,79,11,81]
var nums2 = [5,25,4,39,57,49,93,79,7,8,49,89,2,7,73,88,45,15,34,92,84,38,85,34,16,6,99,0,2,36,68,52,73,50,77,44,61,48]

intersect(nums1, nums2)