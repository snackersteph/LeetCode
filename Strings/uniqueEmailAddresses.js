...
var numUniqueEmails = function(emails) {
  ...
  // normalize all the email addresses 
  let normalizedEmails = emails.map(function(item, index) {
      // split the name and domain of the email
      var [name, domain] = item.split("@")
      
      // get rid of any + signs 
      // we only care about the characters before the +
      // everything after will be ignored
      var [nameNoPlus, namePlus] = name.split("+");
      
      // get rid of any .
      var nameSimplified = nameNoPlus.split(".").join("")
      // compile the full email back together
      var emailSimplified = nameSimplified + "@" + domain;
      
      return emailSimplified;
  })
  
  // put all the entries in a set to make sure they are unique 
